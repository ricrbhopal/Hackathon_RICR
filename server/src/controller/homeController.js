...
export const getAllThemes = async (req, res) => {
  try {
    ...
    const teams = await Team.find({ teamTheme: { $ne: null } }).select(
      "_id teamName teamCode teamTheme createdAt"
    );
    ...
    res
      .status(500)
      .json({ message: "Failed to fetch themes", error: error.message });
  }
};
...
export const ContactUS = async (req, res, next) => {
  try {
    ...
    const EmailContent = `You have received a new message from the contact form on your website.
