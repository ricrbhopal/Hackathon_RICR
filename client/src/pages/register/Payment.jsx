...
const Payment = () => {
  ...
  const referenceId = useMemo(() => {
    ...
    const idpart = registrationData._id
      ? registrationData._id.slice(-8)
  ...
  const handleSubmitProof = async (e) => {
    ...
    if (!utrTrimmed || !utrPattern.test(utrTrimmed)) {
      toast.error(
        "Please enter a valid UPI Transaction ID (UTR) — 12 to 16 digits."
      );
      return;
    }
    if (!registrationData) {
    ...
    try {
      ...
      formData.append(
        "name",
        registrationData.fullName || registrationData.name
      );
      ...
      toast.success(
        "Thank you for registering for Nav Kalpana! Your payment is pending super admin verification. You will receive your User ID and Password by email within 24 hours after super admin approval."
      );
  ...
  const openUpiApp = (app) => {
    ...
    if (!isMobile) {
      toast.info(
        "UPI app links work on mobile. Please scan the QR with your UPI app."
      );
      return;
    }
          ...
          {!paymentSubmitted ? (
              ...
              <div className="flex flex-col items-center bg-gray-50 rounded-xl px-5 py-6 border border-blue-200">
                ...
                {qrDataUrl ? (
                  <img
                    src={qrDataUrl}
                    alt={`UPI QR ₹${AMOUNT}`}
                    className="w-48 h-48 border-2 border-blue-200 rounded-lg object-cover"
                  />
                ) : (
                ...
                <p className="text-sm text-gray-600 mb-2 mt-3 text-center">
                  ...
                  <span className="font-semibold">
                    Amount will be pre-filled to ₹{AMOUNT} (if your app supports
                    it).
                  </span>
                </p>
                ...
                <input
                  ...
                  className={`w-full rounded-lg border px-4 py-2 mb-3 ${
                    utr.length > 0 && !isUtrValid
                      ? "border-red-500 focus:border-red-600 outline-red-500"
                      : ""
                  }`}
                />
                {utr.length > 0 && !isUtrValid && (
                  <div className="text-xs text-red-600 mb-2">
                    Enter a valid UPI Transaction ID (12-16 digits)
                  </div>
                )}
            ...
            <div className="flex flex-col items-center py-8">
              ...
              <h3 className="text-xl font-bold text-blue-700 mb-1">
                Thank you for registering for Nav Kalpana!
              </h3>
              <p className="text-gray-600 mb-4 text-center">
                You will receive your login credentials by email within{" "}
                <span className="font-semibold">24 hours</span> after admin
                approval.
                <br />
                <span className="font-semibold">
                  You cannot access your dashboard until your payment is
                  unverified.
                </span>
              </p>
