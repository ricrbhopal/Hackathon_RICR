...
const OverviewTab = ({ onTabChange }) => {
  ...
  return (
    <div
      className="w-full max-w-5xl mx-auto py-4 sm:py-6 px-3 sm:px-4 lg:px-2 overflow-y-auto scrollbar-thin scrollbar-thumb-blue-300 scrollbar-track-blue-50"
      style={{ maxHeight: "calc(100vh - 48px)" }}
    >
      {/* Header Section */}
      <div className="flex flex-col xs:flex-row xs:items-center xs:justify-between mb-6 sm:mb-8 gap-3 sm:gap-4">
        <h1 className="text-xl sm:text-2xl font-bold text-blue-900 text-center xs:text-left">
          Dashboard Overview
        </h1>
        <button
          onClick={fetchStats}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4 lg:gap-6">
        ...
        >
          <BsMicrosoftTeams
            size={24}
            className="sm:w-6 sm:h-6 lg:w-7 lg:h-7 text-purple-500 mb-1 sm:mb-2"
          />
        ...
        >
          <FiLayers
            size={24}
            className="sm:w-6 sm:h-6 lg:w-7 lg:h-7 text-green-500 mb-1 sm:mb-2"
          />
        ...
        >
          <FiFileText
            size={24}
            className="sm:w-6 sm:h-6 lg:w-7 lg:h-7 text-yellow-500 mb-1 sm:mb-2"
          />
          <div className="text-xl sm:text-2xl lg:text-3xl font-bold text-yellow-700 mb-1">
            {stats.problems}
          </div>
          <div className="text-gray-500 text-xs sm:text-sm font-medium">
